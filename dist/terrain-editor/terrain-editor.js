...
class TerrainEditor {
    ...
    cutWavyHoles(fillGraphic, outer, holes, stamps) {
        if (holes.length === 0 && stamps.length === 0)
            return fillGraphic;
    ...
    // de toda la rejilla.
    collectWavyEraseStamps(layer, memberHexes, bounds) {
        const stamps = [];
        ...
        memberHexes.forEach((hex) => {
            ...
            if (hexData.erasedHoles)
                stamps.push(...hexData.erasedHoles);
        });
        ...
        for (let c = Math.max(0, estimatedColumn - columnSpan); c <= Math.min(this.cols - 1, estimatedColumn + columnSpan); c++) {
            for (let f = 0; f < this.filas; f++) {
                if (memberKeys.has(`${c}:${f}`))
                    continue;
                const hexData = layer.hexes[c][f];
                if (hexData.lastEraseStamp)
                    stamps.push(hexData.lastEraseStamp);
            }
        }
        return stamps;
    }
    ...
    isNearEraserBrush(px, py, stamp, margin) {
        const dx = px - stamp.x;
        const dy = py - stamp.y;
    ...
    isInsideEraserShape(px, py, stamp) {
        const dx = px - stamp.x;
        const dy = py - stamp.y;
        switch (stamp.shape) {
    ...
    eraseAt(x, y) {
        ...
        const isHexFullyCovered = (hexCoordinate) => {
            for (let i = 0; i < hexVertices.length; i += 2) {
                ...
                if (!this.isInsideEraserShape(vx, vy, stamp))
                    return false;
            }
            return true;
        };
        ...
        layer.cities.forEach((city, key) => {
            ...
            if (!this.isNearEraserBrush(hexCoordinate.x, hexCoordinate.y, stamp, 0))
                return;
            city.graphic?.destroy();
            layer.cities.delete(key);
            changed = true;
