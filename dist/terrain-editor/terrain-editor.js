...
class TerrainEditor {
    ...
    getSolidTerrainBelow(hexCoord, maxLayerIndex) {
        for (let i = maxLayerIndex; i >= 0; i--) {
            const layer = this.layers[i];
            if (!layer.visible)
                continue;
            ...
            if (data.paintMode === 'ondulado') {
                if (data.underlyingTerrain)
                    return data.underlyingTerrain;
                continue;
            }
            return data.terreno;
        }
    ...
    drawWavyTerrainGroups() {
        ...
        for (let c = 0; c < this.cols; c++) {
            for (let f = 0; f < this.filas; f++) {
                ...
                if (!visibleTerrain || visibleTerrain.data.paintMode !== 'ondulado' || !visibleTerrain.data.terreno)
                    continue;
    ...
    renderHex(hexCoord) {
        ...
        for (let layerIndex = 1; layerIndex < this.layers.length; layerIndex++) {
            ...
            const data = layer.hexes[hexCoord.col][hexCoord.fila];
            data.graphic.clear();
            if (!layer.visible || !data.terreno || data.paintMode === 'ondulado')
                continue;
