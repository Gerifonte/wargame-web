...
### Borrador
Herramienta aparte (botón "🧹 Borrador"), no un tipo de terreno: no pinta ningún color y no aparece en `TerrainType`. Al activarla se recuerda el pincel que estaba seleccionado y se restaura solo al apagarla. Actúa sobre la **capa activa**; clic izquierdo y arrastrar para borrar, clic derecho sigue moviendo la vista (`eraseAt`). Contorno del pincel visible bajo el cursor (`drawEraserCursor`) para ver el radio y la suavidad antes de pulsar: el contorno completo (marcado) y, más tenue, la banda reducida por la suavidad (`radio * (1 - suavidad/100)`, misma forma y fórmula que el núcleo garantizado de `drawErasedFill`) — igual en las 5 formas. En orgánico/disperso, además, un círculo muy tenue del radio nominal: su propio contorno/manchitas no siempre llegan a marcar ese límite (en disperso ni con suavidad 100%, ya que cada manchita mide como mucho la mitad del radio), así que sin esa referencia no hay forma de calibrar a qué área corresponde el ajuste "Radio" del panel. Todo lo que se puede recortar (hexágonos, perfil libre, río/carretera, perfil ondulado) usa la misma técnica de **hueco geométrico real**: se renderiza la forma junto al hueco (en `blendMode.ERASE`) en un `RenderTexture` propio y aislado (`renderIsolatedWithErase`, resolución ×3 y filtrado suave para que el borde no se vea "de píxeles"; con una forma muy grande la resolución baja para no pedirle a la GPU una textura más grande de lo que admite, y si algo falla se muestra la forma sin recortar en vez de romper el repintado). `Graphics.beginHole`/`endHole` se probó primero pero daba bordes rotos con curvas Bézier.
- **Suavidad = borde difuminado de verdad, en las 5 formas** (`drawErasedFill`): 32 anillos concéntricos con opacidad parcial (menos se veían como bandas duras, efecto "a rodajas"), que al superponerse (blendMode ERASE reduce opacidad de forma multiplicativa, no es un corte binario) dan un degradado real, más "mordido" cerca del centro. Funciona igual para las 5 porque `drawEraserStampShape` solo depende linealmente de `stamp.radius` (círculo/cuadrado/diamante escalan directo; `buildOrganicPolygon`/`buildDispersoBlobs` también escalan con él), así que encoger el radio anillo a anillo reduce toda la silueta manteniendo su forma. En orgánico/disperso la suavidad hace además lo de siempre: más irregular el bamboleo o más repartidas las manchitas (`drawEraserStampShape`) — las dos cosas a la vez, no una u otra.
...
- **Perfil ondulado**: el contorno de un grupo de hexágonos se recalcula desde los datos de esos hexágonos en cada repintado, así que borrar (o simplemente no pintar) un hexágono en medio de una zona rodeada por el resto del grupo deja un agujero real, no solo en el borde. El contorno exacto por aristas (`createGroupContours`) distingue "borde exterior de una isla" de "borde de un agujero" por el sentido de giro del polígono (`signedArea`: fiable con formas cóncavas, a diferencia de mirar dónde cae el centro). Ese agujero interior se recorta como polígono exacto (no es un hueco del pincel, así que no lleva difuminado), pero en el mismo recorte (`cutWavyHoles`) se añaden también los sellos del borrador que hayan tocado la mancha (`collectWavyEraseStamps`, forma y suavidad reales vía `drawErasedFill`): tanto los de hexágonos miembros solo rozados en parte (`HexData.erasedHoles`) como el último sello que vació del todo un hexágono vecino (`HexData.lastEraseStamp`, se conserva aunque el hexágono quede vacío y se limpia al repintar esa casilla) — así que el borrado se comporta igual que en hexágonos sueltos/perfil libre/río/carretera. Aplica tanto a "Una mancha por arrastre" (envolvente convexa) como a "Una mancha por grupo" (cada isla con su forma exacta).
...
- **Formas del pincel**: círculo, cuadrado, diamante y dos formas irregulares — **orgánico** (contorno desigual tipo borde desgarrado) y **disperso** (varias manchitas sueltas). Las 5 llevan difuminado real con la suavidad; en orgánico/disperso, además, la suavidad controla lo irregular/repartido del hueco. Cada hueco guarda su propia semilla y suavidad (`EraserStamp`) para no deformarse si luego se cambia el ajuste global.
  - `isHexFullyCovered` (¿se vacía el hexágono del todo?) usa la forma **real** de orgánico/disperso (`isInsideEraserShape` con `buildOrganicPolygon`/`buildDispersoBlobs`, compartidas con el dibujo) y no un círculo genérico del radio nominal — si no, un hexágono se vaciaba del todo por "caber en el círculo" aunque el polígono/las manchitas dibujadas fueran más pequeñas ahí, y la suavidad/forma no se notaba nunca (el hexágono desaparecía limpio antes de llegar al hueco parcial con difuminado).
  - El resto de comprobaciones (¿toca este hexágono como para darle un hueco parcial?, ¿toca esta casa/punto de trazo?) siguen usando un alcance amplio tipo círculo/cuadrado/diamante (`isNearEraserBrush`, el criterio de siempre) **sin** la forma exacta de orgánico/disperso: son un filtro barato antes de dibujar (o, en casas/tren, no tienen un dibujado parcial posterior que corrija la forma), así que exigir ahí la forma exacta dejaba casi todo sin tocar en disperso (manchitas diminutas) aunque el pincel sí pasara por encima.
- **Espaciado mínimo entre sellos** (`eraseAt`, ver `lastEraseX`/`lastEraseY`): un arrastre dispara un borrado por cada `pointermove`, así que sin límite un arrastre lento sobre un borde que nunca llega a cubrir el hexágono entero (más fácil con orgánico/disperso, cuyo borde efectivo es menor que el radio nominal) apila cientos de sellos en el mismo hexágono/trazo y revienta la memoria al repintar. Un nuevo sello solo se añade si el cursor se ha movido al menos un 25 % del radio del pincel desde el último.
- **Tope de sellos por hexágono/trazo** (`appendErasedHole`, `MAX_ERASED_HOLES_PER_TARGET` = 24): el espaciado de arriba limita los sellos por distancia recorrida, pero un borrado **continuo** (arrastrar de un lado a otro sobre la misma zona un buen rato) los sigue acumulando sin límite con el tiempo. Con difuminado (ver más abajo, aplica a las 5 formas) cada sello son 33 anillos, así que sin tope la geometría acumulada podía reventar la memoria iterando. Por encima del tope se descartan los sellos más viejos (no el nuevo), para que el borrado siga notándose.
...
- Lo último implementado (23/09/2026): el borrado de **perfil ondulado** ya se comporta igual que el resto de herramientas del Borrador (hexágonos, perfil libre, río/carretera) — ver sección "Borrador" más arriba para el detalle técnico. De paso, **orgánico y disperso pasaron a tener difuminado real** (antes solo cambiaban lo irregular/repartido de su forma, sin desenfoque de verdad), y se corrigieron dos crashes por acumulación sin límite de sellos de borrado al arrastrar/borrar de forma continua (espaciado mínimo por distancia + tope de sellos por hexágono/trazo).
- Todo lo de esta sesión está **sin commitear** (`js/terrain-editor.ts`, `dist/terrain-editor/terrain-editor.js` y este README). Nada de `Editor-Terreno.php` ni `css/terrain-editor.css` cambió esta vez.
- Verificado en navegador (Edge headless) durante la sesión: perfil ondulado con las dos agrupaciones, las 5 formas del borrador con distintas suavidades, borrado continuo/repetido sin cuelgues, y un caso de "círculos verdes sueltos" que resultó ser la Capa Base asomando por los agujeros (comportamiento esperado, no bug). Pendiente que el usuario lo confirme también en su propio navegador con recarga forzada.
- Siguiente paso natural: seguir con lo que quedó pendiente de antes (revisar visualmente los pinceles nuevos, decidir cómo debe ser **Pueblo**), salvo que el usuario traiga algo nuevo sobre el Borrador.
